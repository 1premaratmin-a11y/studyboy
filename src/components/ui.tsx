...
export function Ptag({ children, tone = "default" }: { children: ReactNode; tone?: "default" | "ink" | "blue" | "cyan" | "ember"; }) {
  const cls = tone === "blue" || tone === "cyan" ? "tag tag-accent" : tone === "ember" ? "tag tag-danger" : tone === "ink" ? "tag" : "tag";
...
export function HudCell({ icon, value, title }: { icon: ReactNode; value: ReactNode; title: string; }) {
  return (
    <div title={title} className="flex items-center gap-1.5 bg-surface2 border border-border px-2.5 py-1 rounded text-xs text-primary">
...
export function Donut({ pct, size = 64, color = "var(--accent)", track = "var(--surface-3)" }: { pct: number; size?: number; color?: string; track?: string; }) {
  ...
  return (
    <svg width={size} height={size} viewBox="0 0 64 64" aria-label={`${pct}% complete`}>
      ...
      <circle cx="32" cy="32" r={r} fill="none" stroke={color} strokeWidth="6" strokeDasharray={`${dash} ${c}`} strokeLinecap="round" transform="rotate(-90 32 32)" />
    </svg>
  );
}
...
export function MasteryRamp({ level }: { level: 0 | 1 | 2 | 3; }) {
  return (
    <div className="flex gap-1 col-span-full">
      {[0, 1, 2, 3].map((i) => (
        <div key={i} className={`flex-1 h-1.5 rounded-full ${i <= level ? "bg-accent" : "bg-surface3"}`} />
      ))}
    </div>
  );
}
...
export function Heatmap({ cells, cols = 26, rows = 7 }: { cells: { date: number; minutes: number }[]; cols?: number; rows?: number; }) {
  ...
  return (
    <div className="flex gap-2.5 items-start">
      <div role="img" aria-label={`${cols}-week study heatmap, ${Math.round(total / 60)} total hours`}
        className="grid gap-[3px]" style={{ gridTemplateRows: `repeat(${rows}, 13px)`, gridAutoColumns: "13px", gridAutoFlow: "column" }}>
        {cells.slice(-cols * rows).map((c, i) => (
          <div key={i} aria-hidden="true" title={`${new Date(c.date).toLocaleDateString()} · ${c.minutes}m`}
...
export function FocusScreen({ time, session, pipsOn, pipsCur, label, armed, onStart, onPause, mini }: {
  ...
  return (
    <div className="card p-4 h-full flex flex-col">
      ...
      <div className="font-mono text-5xl text-primary text-center my-2 tracking-wide">{time}</div>
      <div className="flex gap-1.5 justify-center my-2">
        {[0, 1, 2, 3].map((i) => (
          <span key={i} className={`w-2 h-2 rounded-full ${i < pipsOn ? "bg-accent" : i === pipsCur ? "bg-warning" : "bg-surface3"}`} />
        ))}
      </div>
      <div className="flex gap-2 justify-center mt-3"><PixelButton armed={armed} onClick={onStart}>Start</PixelButton><PixelButton onClick={onPause}>Pause</PixelButton></div>
...
export function Toast({ title, body, onClose, style }: { title: string; body: string; onClose: () => void; style?: CSSProperties; }) {
  return (
    <div role="status" aria-live="polite" className="fixed top-20 right-4 z-[500] max-w-xs rounded-lg flex gap-2 items-start p-3"
      style={{ background: "rgba(19,19,24,0.95)", backdropFilter: "blur(12px)", border: "1px solid rgba(245,158,11,0.3)", boxShadow: "0 4px 16px rgba(0,0,0,0.4)", ...style }}>
      ...
      <button onClick={onClose} aria-label="Dismiss notification" className="text-muted hover:text-primary text-sm bg-transparent border-0 p-0 leading-none ml-1">×</button>
    </div>
  );
}